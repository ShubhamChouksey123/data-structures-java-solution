...
public class StreamsExample {
    ...
    public static void main(String[] args) {
        ...
        personList.stream().filter((p) -> p.getLastName().startsWith("C")).forEach(System.out::println);

        long count = personList.stream().filter((p) -> p.getLastName().startsWith("C")).count();
        System.out.println("count : " + count);

        long countUsingParallelStream = personList.parallelStream().filter((p) -> p.getLastName().startsWith("C"))
                .count();
        ...
        numbers.stream().sorted(Comparator.reverseOrder()).distinct().forEach(number -> System.out.print(number + " "));

        System.out.println("\ncollecting the stream result ");
        List<Integer> l = numbers.stream().sorted(Comparator.reverseOrder()).distinct().collect(Collectors.toList());
